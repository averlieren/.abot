...
UserProfile = new (require path.join __dirname, 'user')
...
class Tag
  parse: (message) ->
    ...
    return undefined if !game || game == 'here' || game == 'everyone'
  ...
  getUsers: (game, message) ->
    ...
    for member in message.guild.members.values()
      continue if member.id == message.author.id || message.author.bot
      for user of users
        console.log "[.abot8] #{user}"
        continue if user.options.tag != 'true'
